...
# Purpose: train and evaluate LSTM-CP baseline for the 3 official cpu_precursor_v1 runs.
...
function Invoke-TrainEvalLstm {
    param([string]$Config, [string]$RunDir)

    python src\train.py --config $Config --model lstm --resume_dataset_run $RunDir --variant $VARIANT
    python src\evaluate.py --config $Config --model lstm --resume_dataset_run $RunDir --variant $VARIANT
}

Invoke-TrainEvalLstm -Config $CONFIG42 -RunDir $RUN42
Invoke-TrainEvalLstm -Config $CONFIG2025 -RunDir $RUN2025
Invoke-TrainEvalLstm -Config $CONFIG3407 -RunDir $RUN3407
